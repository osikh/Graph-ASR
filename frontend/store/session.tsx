...
import type { AgentEvent, SysLog } from "@/types";

// ── live graph types (positions assigned in frontend) ────────────────────────
...
// ── position helper — golden-angle spiral so nodes spread naturally ──────────
...
const NODE_RADIUS: Record<string, number> = {
  concept: 20, evidence: 14, claim: 16, gap: 17, answer: 16,
};
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const connectWs = useCallback((sid: string) => {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.type === "sys_event") {
        setSysLogs(prev => [...prev, { t: msg.t, log: msg.log }]);
      }
        ...
        if (msg.node) {
          ...
          setNodes(prev => {
            ...
            return [...prev, {
              ...
              type:  msg.node.type,
              ...pos,
              r: NODE_RADIUS[msg.node.type] ?? 16,
              t: (Date.now() - startRef.current) / 1000,
            }];
          });
        }
        if (msg.edge) {
          setEdges(prev => [...prev, {
            from: msg.edge.from_id,
            to:   msg.edge.to_id,
            type: msg.edge.type,
            t:    (Date.now() - startRef.current) / 1000,
          }]);
  ...
  return (
    <SessionCtx.Provider value={{
      sessionId, status, question, events, sysLogs,
      confidence, nodes, edges, elapsed, submit,
    }}>
      {children}
    </SessionCtx.Provider>
  );
